    ...
    <main className="flex-grow">
      ...
      <section className="px-4 pt-5 pb-5">
        <div className="flex items-center justify-between"> {/* Flex container for title and button */}
          <div>
            <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Reminders</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1"> Scheduled payment and follow-up alerts </p>
          </div>
          <Link
      ...
      <section className="px-4 pb-20"> {/* Added pb-20 for clearance above the sticky footer */}
        <div className="space-y-3">
          {invoices.map((invoice) => (
            <Link href={`/invoice/${invoice.id}`} key={invoice.id} className="block rounded-xl bg-white p-4 border border-slate-200 transition-shadow hover:shadow-md dark:bg-slate-800 dark:border-slate-700">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">
                  # <span className="font-bold text-slate-700 dark:text-slate-300">{invoice.id}</span>
                </p>
                <div className="flex items-center gap-2">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${invoice.statusClass}`}>
                    {invoice.status}
                  </span>
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Due: {new Date(invoice.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </p>
              </div>
            </Link>
          ))}
